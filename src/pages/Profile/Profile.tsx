...
import { IOrder, IUser, IReview, ISuggestion } from "../../interfaces/Interfaces";
...
export const Profile: FC = () => {
    ...
    const suggestions: ISuggestion[] = JSON.parse(localStorage.getItem("suggestions") || "[]");
    const readerReviews = ReaderReviewsData;
    const currentUserEmail: string | null = isLoggedIn ? currentUser[0].email : null;
    const currentUserFirstName: string | null = isLoggedIn ? currentUser[0].firstName : null;
    const currentUserLastName: string | null = isLoggedIn ? currentUser[0].lastName : null;

    const orderExists = orders.find((order: IOrder) => {
        return order.user.email === `${currentUserEmail}`;
    });

    const hardCodedReviewHistory = readerReviews.filter((review: IReview) => {
    ...
    const reviewHistory = reviews.filter((review: IReview) => {
        return review.user.email === `${currentUserEmail}`;
    }).map((review: IReview) => {
        return <ReviewHistory
            key={review.id}
    ...
    const suggestionHistory = suggestions.filter((suggestion: ISuggestion) => {
